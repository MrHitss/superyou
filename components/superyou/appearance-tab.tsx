...
function layoutToAppearance(layout: Record<string, unknown> | undefined): Partial<AppearanceState> {
  ...
  return {
    ...
    backgroundType: (background?.type as 'solid' | 'pattern' | 'image') === 'color' ? 'solid' : (background?.type as 'solid' | 'pattern' | 'image') ?? DEFAULT_APPEARANCE.backgroundType,
  ...
  const renderPreviewCard = (card: typeof previewCards[0], index: number) => {
    ...
    return (
      <div
        ...
        className={cn(
          'relative shrink-0 transition-all duration-300 cursor-pointer',
          isActive ? 'w-[300px] opacity-100 scale-100 z-20' : 'w-[240px] opacity-25 scale-90',
        )}
        style={{
      ...
      >
        ...
        >
          ...
          <div className={cn('h-32 flex flex-col items-center justify-center pt-6', isActive ? 'bg-transparent' : 'bg-gray-200')}>
            <div className={cn('h-16 w-16 rounded-full border-2 flex items-center justify-center mb-2', isActive ? 'bg-black/30 border-white/30' : 'bg-gray-300 border-gray-400')}>
              <span className={cn('text-2xl', isActive ? 'text-white/90' : 'text-gray-500')}>👤</span>
            </div>
            <p
              className={cn('font-semibold text-base', isActive ? 'text-white' : 'text-gray-600')}
              style={isActive ? { color: appearance.pageFontColor, fontFamily: appearance.font, textShadow: '0 1px 2px rgba(0,0,0,0.5)' } : {}}
            >
              Hitesh Varyani
            </p>
            <p className={cn('text-xs mt-0.5', isActive ? 'text-white/90' : 'text-gray-500')} style={isActive ? { textShadow: '0 1px 2px rgba(0,0,0,0.5)' } : undefined}>
              Welcome to my SuperProfile! 🚀
            </p>
            <div className={cn('mt-2 h-8 w-8 rounded-full flex items-center justify-center', isActive ? 'bg-white/20 backdrop-blur-sm' : 'bg-gray-400')}>
              <span className="text-white text-[10px] font-semibold">IG</span>
            </div>
          </div>

          {/* Middle section - transparent so card background shows */}
          <div className={cn('h-32 flex items-center justify-center', isActive ? 'bg-transparent' : '')} style={!isActive ? { background: 'linear-gradient(180deg, #e5e5e5 0%, #d4d4d4 100%)' } : undefined}>
            <div
              className={cn(
            ...
            >
              <span className={cn('text-xl font-semibold absolute bottom-1', isActive ? 'text-white/90' : 'text-gray-500')} style={isActive ? { textShadow: '0 1px 2px rgba(0,0,0,0.5)' } : undefined}>n</span>
            </div>
          </div>

          {/* Bottom section - transparent so card background shows; block card uses blockColor */}
          <div className={cn('flex-1 p-4 min-h-[120px]', isActive ? 'bg-transparent' : 'bg-[#f5f5f5]')}>
            <div
              className={cn(
            ...
            >
              <p className={cn('font-medium text-sm', isActive ? '' : 'text-gray-600')}>
                Get the 30 day Deep-dive E-Book
              </p>
              <p className={cn('text-xs mt-0.5', isActive ? 'opacity-90' : 'text-gray-500')}>
        ...
        {isActive && (
          <p className="text-center text-sm font-semibold mt-2 text-foreground">{card.name}</p>
        )}
      </div>
    );
  };
  ...
  return (
    <div className="flex flex-col min-h-full bg-gray-100">
      ...
      <div className="flex items-center justify-between px-6 py-4">
        <div className="bg-white rounded-full px-3 py-2 flex items-center gap-3 shadow-sm border">
        ...
        <DropdownMenu open={fontModalOpen} onOpenChange={setFontModalOpen}>
          <DropdownMenuTrigger asChild>
            ...
            >
              ...
              <span className="text-sm text-gray-600">{appearance.font}</span>
              <div className="flex flex-col -space-y-0.5">
                <ChevronUp className="h-2.5 w-2.5" />
                <ChevronDown className="h-2.5 w-2.5" />
              </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" onClick={handleSave} disabled={!hasUnsavedChanges} className="rounded-full px-4">
            Save changes
          </Button>
        </div>
      </div>
      ...
      {/* Preview Carousel */}
      <div className="flex-1 flex items-center justify-center overflow-x-auto px-6 py-8">
        <div className="flex items-center gap-0 justify-center relative" style={{ maxWidth: '100%' }}>
